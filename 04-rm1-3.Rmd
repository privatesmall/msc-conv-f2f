...
* Download <a href="files\4.2.zip" download> the chapter 4.2 files</a> and extract the files in to your Chapter 4 folder. 
* Open R Studio and ensure the environment is clear.  
* Open the `stub-4.2.Rmd` file and ensure your working directory is set to your Chapter 4 folder and that you can see the data files in the file pane.  
...
* Replace the `NULL` values in the below code with the necessary variable names to join `rlong1` and `qformats` by `Question`. If you need extra help, revisit Chapter 2 In-class Activity 5 - you used the same function then! You can also check the solutions for the answer (but make sure you try yourself first).
...
* Open R Studio and ensure the environment is clear.   
* Download <a href="files\4.3.zip" download> the chapter 4.3 files</a> and extract the files in to your Chapter 4 folder. 
* Open the `stub-4.3.Rmd` file and ensure that the working directory is set to your Chapter 4 folder and that you can see the data files in the file pane.
...
How many participants in the control condition didn't donate? `r fitb("22")`  
How many participants in the control condition donated? `r fitb("16")`  
How many participants in the rta condition didn't donate? `r fitb("10")`  
How many participants in the rta condition donated? `r fitb("29")`  
...
```{r percent_table, echo = FALSE}
intent_percent %>% knitr::kable(digits = 2, align = "c")
```
...
```{r layer1, fig.cap="Bar plot of RTA Study 1 data"}
...
```{r plot_edits, fig.cap = "Prettier bar plot of RTA Study"}
...
1. What is the chi-square statistic? `r fitb("8.244")`
2. Is the p-value significant? `r mcq(c(answer = "Yes", "No"))`  
3. What are the degrees of freedom for the test? `r fitb("1")`  

`r hide("Explain these answers")`
```{r, echo = FALSE, results='asis'}
cat("1. The chi-square statistic is noted in the output as X-squared. Refer to the lecture for more information on how this number is calculated.  
...
```{r img-lists, echo=FALSE, fig.cap="Contents of a list"}
...
Does the data meet assumption 6? `r mcq(c(answer = "Yes - all expected frequencies are > 5","No - one or more expected frequencies are < 5"))`
...
> Those in the reminder-through-association condition performed the intended behaviour at a significantly higher rate (`r round(pluck(intent_percent$percent_condition, 3),0)`%, `r pluck(intent_percent$n, 3)` out of `r pluck(intent_percent$n, 3) + pluck(intent_percent$n, 4)`) than did those in the control condition (`r round(pluck(intent_percent$percent_condition, 1),0)`, `r pluck(intent_percent$n, 1)` out of `r pluck(intent_percent$n, 1) + pluck(intent_percent$n, 2)`)), χ2(`r results$parameter`, N = `r length(intent_recode$id)`) = `r round(results$statistic,2)`, p = `r round(results$p.value, 3)`,V = `r round(eff_size, 2)`.
...
1. You have a dataset where gender has been coded numerically. You want to recode this to use text labels. Which code will work? `r mcq(c("mutate(gender = recode(gender, \"male\" = \"1\", \"female\" = \"2\", \"nonbinary\" = \"3\"))", "mutate(gender = recode(gender, male = 1, \"female = 2, nonbinary = 3))", answer = "mutate(gender = recode(gender, \"1\" = \"male\", \"2\" = \"female\", \"3\" = \"nonbinary\"))"))`

`r hide("Explain this answer")`
```{r, echo = FALSE, results='asis'}
cat("The first option has the new and old codes in the wrong position, the second option is missing quotation marks, the third option is correct.
    ")
```
`r unhide()` 
...
2. From the below code, what would the plot have on the x-axis? `r mcq(c("exp_data", answer = "gender", "score"))`

```{r, eval = FALSE}
ggplot(data = exp_data, aes(gender, score))
```

3. From the below code, how would the bars in the plot be positioned? `r mcq(c(answer = "On top of each other", "Next to each other"))`  
...
cat("If you want the bars to be next to each other you would need to add `position = "dodge" to `geom_bar()`.
    ")
```
`r unhide()` 
...
4. Which of the following is not an argument of `chisq.test()` (you may need to look at the help documentation to answer this question)? `r mcq(c("x", "y", "p", answer = "continuity"))`
