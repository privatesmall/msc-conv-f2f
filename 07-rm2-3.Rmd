...
```{r facet1, fig.cap = "Violin-boxplot facetted by gender"}
ggplot(zhang_data, aes(x = Condition, y = interest, fill = time))+
  geom_violin(alpha = .6, trim = FALSE)+
...
```{r sumdat2, fig.cap = "Violin-boxplot facetted by gender with summary data"}
...
* Think back to the t-test lab. What is the name of the function for calculating Cohen's D? `r fitb("cohensD")`. What package does this come from? `r fitb("lsr")`
...
* Say you run the study and find that the effect size determined is r = .24. Given what you know about power, select the statement that is true: `r mcq(c("the study is sufficiently powered as the analysis indicates you can detect only effect sizes smaller than r = .24",answer = "the study is underpowered as the analysis indicates you can detect only effect sizes larger than r = .34"))`
